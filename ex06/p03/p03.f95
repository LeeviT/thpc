program p03
  ...
  if (id .eq. ntasks-1) call mpi_send(msg, 1, mpi_integer, 0, tag, &
                                      mpi_comm_world, rc)
  if (id .ne. ntasks-1) write (*,*) 'sending process id:', id, &
                                    'receiving process id:', id+1
  if (id .eq. ntasks-1) write (*,*) 'sending process id:', id, &
                                    'receiving process id:', '0'
  call mpi_barrier(mpi_comm_world,ierr)
  call mpi_recv(msg, 1, mpi_integer, mpi_any_source, tag, mpi_comm_world, status, rc)
  write (*,*) 'sending process id:', status(mpi_source), &
